...
def scrappingnews(browser_name) : 
    ...
    # 뉴스명 ====> (개인별 수정 필요!)
    news_comany = '헬스조선'
    coll_newsscrap = dbconnect("healthjosun02")
    ...
    for element in element_body :
        try :
            ...
            # 스크래핑 시작 ----> (개인별 수정 필요!)
            time.sleep(1)
            news_title = browser.find_element(by=By.CSS_SELECTOR, value="div > h1").text
            news_description_body = browser.find_element(by=By.CSS_SELECTOR, value="div.par > div:nth-child(2)").text
            # news_description = ''
            # try : 
            ...
            #     pass
            news_time = browser.find_element(by=By.CSS_SELECTOR, value="div > dl.wrt-text").text
        except NoSuchElementException:
            pass

        coll_newsscrap.insert_one({'news_company' : news_comany, 'news_time' : news_time, 'news_title' : news_title, 'news_description' : news_description_body})
        browser.close()
        browser.switch_to.window(origin_tab)


if __name__ == "__main__" :
    scrappingnews("https://search.naver.com/search.naver?where=news&query=%ED%9D%AC%EA%B7%80%EC%A7%88%ED%99%98&sm=tab_opt&sort=0&photo=0&field=0&pd=5&ds=2023.02.27&de=2024.02.27&docid=&related=0&mynews=1&office_type=1&office_section_code=8&news_office_checked=1346&nso=so%3Ar%2Cp%3A1y&is_sug_officeid=0&office_category=0&service_area=0")